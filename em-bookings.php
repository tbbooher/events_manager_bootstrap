	...
	function create() {                
		...
		<div id="em-booking">
			...
			<?php if( !empty($EM_Event->get_bookings()->feedback_message) && count($EM_Event->get_bookings()->errors) == 0 ) : ?>
				<div class='bg-success'><?php echo $EM_Event->get_bookings()->feedback_message; ?></div>
			<?php elseif( count($EM_Event->get_bookings()->errors) > 1 ) : ?>
				<div class='bg-warning'><?php echo implode('<br />', $EM_Event->get_bookings()->errors); ?></div>
			<?php elseif( !empty($EM_Event->get_bookings()->feedback_message) ) : ?>
				<div class='bg-info'><?php echo $EM_Event->get_bookings()->feedback_message; ?></div>
			<?php endif; ?>
			...
			<form id='em-booking-form' name='booking-form' method='post' action=''>
				...
				<div class="em-booking-buttons">
					<?php echo apply_filters('em_booking_form_buttons', '<input type="submit" class="btn btn-primary" value="'.__('Send your booking', 'dbem').'" />'); ?>
			...
			jQuery(document).ready( function($){
				$('#em-booking-form').ajaxForm({
					...
					success : function(response, statusText, xhr, $form) {
						$('#em-loading').remove();
						if(response.result){
							$('<div class="bg-success em-booking-message">'+response.message+'</div>').insertBefore('#em-booking-form');
						}else{
							$('<div class="bg-warning em-booking-message">'+response.message+'</div>').insertBefore('#em-booking-form');
	...
	function cancel() {   
		...
		<div id="em-booking-delete">
			...
			<?php if( is_user_logged_in() ): ?>
				...
				<div class='bg-success'><?php echo self::$em_form_messages_booking_cancel['success'] ?></div>
				<?php elseif( !empty(self::$em_form_messages_booking_cancel['error']) ) : ?>
				<div class='bg-warning'><?php echo self::$em_form_messages_booking_cancel['error'] ?></div>
				<?php elseif( !empty(self::$em_form_messages_booking_cancel['message']) ) : ?>
				<div class='bg-info'><?php echo self::$em_form_messages_booking_add['message'] ?></div>
				<?php endif; ?>
